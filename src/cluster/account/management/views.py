...
from cluster.account.management.forms import IntroPageForm, ProposalUploadForm
...
@login_required
def proposal_sample(request):
    if not PermissionController.is_admin(request.user):
        return HttpResponseRedirect('/login/')
    instance = IntroPageContent.get_instance()
    if request.method == 'POST':
        form = ProposalUploadForm(request.POST,request.FILES, instance=instance)
        if form.is_valid():
            form.save()
            messages.success(request, u"ویرایش اطلاعات با موفقیت انجام شد.")
        else:
            messages.error(request,u"فرم نا معتبر است.%s"%form.errors)
    else:
        form = ProposalUploadForm(instance=instance)

    return render_to_response('accounts/proposal_sample_upload.html',{"form":form},context_instance=RequestContext(request))
